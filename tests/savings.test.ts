...
describe("UsageDB.record — saver fields", () => {
  ...
  it("persists saver fields when provided", () => {
    ...
    d.record({
      ...
      caveman_level: "full", ponytail_level: "lite",
    });
    const row = d.recent(1)[0]!;
...
describe("UsageDB.savingsSummary", () => {
  it("sums RTK byte savings and counts only rows RTK actually touched", () => {
    const d = db();
    d.record({ alias: "a", provider: "p", model: "m", tokens_in: 1, tokens_out: 1, cached_tokens: 0, cost: 0, status: 200, latency_ms: 1, stream: 0, rtk_bytes_in: 1000, rtk_bytes_out: 400 });
    d.record({ alias: "a", provider: "p", model: "m", tokens_in: 1, tokens_out: 1, cached_tokens: 0, cost: 0, status: 200, latency_ms: 1, stream: 0, rtk_bytes_in: 2000, rtk_bytes_out: 1000 });
  ...
  it("sums Headroom token savings the same way", () => {
    const d = db();
    d.record({ alias: "a", provider: "p", model: "m", tokens_in: 1, tokens_out: 1, cached_tokens: 0, cost: 0, status: 200, latency_ms: 1, stream: 0, headroom_tokens_before: 500, headroom_tokens_after: 200 });
