...
/** Human window: "1day" → "every day", "5h" → "every 5 hours", "7day" → "every 7 days". */
function windowPeriod(w: string): string {
...
/** Short chip: "1day" → "1 day", "5h" → "5h". */
function windowChip(w: string): string {
  const m = /^(\d+)(h|day)$/.exec(w);
  if (!m) return w;
  const n = Number(m[1]);
  if (m[2] === "h") return `${n}h`;
 ...
 * Budget windows always refill — separate from key expiry.
 */
  ...
  return (
    <div className="mb-4 overflow-hidden rounded-brand-lg glass-premium">
      ...
      <div className="grid sm:grid-cols-3">
        {/* Budget — always a refilling window, never "lifetime until key dies" */}
          ...
          {b ? (
            <>
              ...
              <div className="mt-1 text-[13px] font-medium text-text">
                ...
                <span className="font-normal text-text-muted">{windowPeriod(b.window)}</span>
              </div>
              ...
              <div className="mt-1.5 space-y-0.5 text-[12px] text-text-muted">
                <div>
                  {formatBudgetAmount(b.spent, b.unit)} used this {windowChip(b.window)} window
                ...
                <div className="text-[11px] text-text-subtle">
                  Not a one-time total — separate from key expiry below.
                </div>
              </div>
            </>
          ) : (
        ...
        {/* Key expiry + RPM — not the budget window */}
          ...
          {me.expires ? (
            <>
              ...
              <div className="mt-1.5 text-[11px] text-text-subtle">
                After this date the key dies. Budget still refills until then.
              </div>
            </>
          ) : (
            <>
